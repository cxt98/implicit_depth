...
class ClearPose_Object_Dataset(Dataset):
    def __init__(self, dataset_subdir, exp_type, params):
        ...
        print('{} images for clearpose dataset'.format(len(self.image_paths)))
    ...
    def list_dataset(self, data_path):
        ...
        opaque_depth_paths = []
        camera_intrinsics = {'fx': 601.3, 'fy': 601.3, 'cx': 334.7, 'cy': 248.0, 'xres': 640, 'yres': 480}
        for set_scene in self.params['set_scenes']:
            path = f'{data_path}/set{set_scene[0]}/scene{set_scene[1]}/'
            # collect transparent rgb, mask, depth paths
            cur_image_paths = sorted(glob(osp.join(path, '*-color.png')) )
    ...
    def __getitem__(self, idx):
        ...
        ## corrupt mask seems to be the mask with invalid/inaccurate depth measurement
        corrupt_mask = cv2.imread(labels_filename, -1)
        corrupt_mask[corrupt_mask != 0] = 1
        corrupt_mask = cv2.resize(corrupt_mask, (self.params['img_width'], self.params['img_height']), interpolation=cv2.INTER_NEAREST)
        ...
        # # scale depth and xyz
        depth_corrupt_img = cv2.imread(self.transparent_depth_paths[idx], -1).astype(np.float32) / 1000. 
        ...
        depth_img = cv2.imread(self.opaque_depth_paths[idx], -1).astype(np.float32) / 1000. 
        ...
        item_path = rgb_filename.replace('-color.png', '')
...
def get_dataset(dataset_dir, params, exp_type, obj_type='known'):
    ...
    # set params
    params = params.copy()
    params['set_scenes'] = set_scenes
    if exp_type != 'train':
        params['use_data_augmentation'] = False
    dataset = ClearPose_Object_Dataset(dataset_subdir, exp_type, params)
    return dataset
